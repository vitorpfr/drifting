...
| Startup station mode | ✅ Done — "First station: Auto / From saved" setting in Settings; `startupStationMode: 'auto' \| 'saved'` in `UIPreferencesContext` (localStorage `startup_station_mode`, default `'auto'`); `getStartupStation` callback option in `useAudioLifecycle` called once in `loadInitial` before the preloaded station is used; falls back to `auto` silently when saved list is empty; deep link always overrides; retry after failed startup station falls back to `pickNext` |
