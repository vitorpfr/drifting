...
| Connectivity handling / retry | ✅ Done — orchestrator pattern in `useAudioLifecycle`: both `loadInitial` and `goNext` retry indefinitely (while-true loop, 5s per attempt) until a station connects; each failure passes the failed station as `lastAttempted` to `pickNext` so every retry picks a different station; second tap while connecting is a no-op (orchestrator guarantees a station will land); brief "Connection failed, trying next station…" flash shown during `goNext` retries (silent on initial load); **stall watchdog (two-window grace):** measures zero audio progress after a station commits — a freshly-committed station gets a one-time `FIRST_BUFFER_GRACE_MS` (12s) window for its *first* stall (live streams often emit `waiting` while filling their initial buffer on marginal connections, and the old flat 5s declared them "dropped" too early — the main source of false auto-advances), and `STALL_TIMEOUT_MS` (8s, was 5s) for any stall after the station has already played; **deliberate-only notify:** the watchdog auto-advance shows a notice *only* when abandoning a station the user deliberately chose — deep link, a pick from search / the saved shelf, or a `goPrevious` re-hear — with a named message `"Lost connection to {name}, moving on…"` (single persistent notice, dismissed by the App's visibility-aware 4s timer); algorithmic discovery picks (`goNext`/startup/random) move on **silently** (the old generic "stream dropped, moving…/moved" toast is gone); selection-source tracked via `deliberateSelectionRef`, set in `commitStation(s, p, deliberate)`; session-level `AudioContext` singleton (`sessionAudioContext.ts`) shared across all `AudioPlayer` instances so iOS Safari never needs to resume it again after the first gesture; auto-resume on network recovery; page visibility recovery |
...
| Visibility-aware transient UI | ✅ Done (PR #33) — shared `useVisibilityAwareTimeout` hook: auto-dismiss countdown only elapses while the page is visible (pauses when the tab is backgrounded / phone locked, resumes the remainder on return; re-arms per `restartKey`, always calls the latest callback). Applied to `Toast` (now visibility-aware by default — the per-toast opt-in was removed), `KofiNudge` (5s), and the stall notice (4s timer; copy since reworked to the deliberate-only `"Lost connection to {name}, moving on…"` — see "Connectivity handling / retry"). Keeps transient messages from being spent before the user sees them |
...
| Programmatic SEO — station pages (batch 1) | ✅ Done (PR #36 + polish #37) — **diagnostic 100-station rollout** to close the organic-search gap (GSC pre-launch: 9 impressions/month, all branded — zero topical surface area). Server-rendered crawlable pages via Cloudflare Pages Functions (same on-demand pattern as `functions/og.ts`, no build-time file explosion, scales to full catalog in batch 2). **Pages:** `/radio/<slug>-<uuid>` (keyword `<title>`/`<h1>`, varied non-boilerplate description, facts country·genre·bitrate + HQ badge, JSON-LD `RadioStation`, canonical, OG/Twitter reusing `/og` image, station-color radial glow + favicon tile, "▶ Listen on Spectrale" deep-link → existing `/?station=<uuid>`, ≤6 related-station internal links); `/stations` master index (all 100, color-dot rows) linked from homepage noscript; `sitemap.xml` regenerated (103 URLs, was 2). **Selection blend** (`scripts/buildStationPages.ts`, `npm run build:station-pages`): top-voted, stream-verified, deduped-by-name — ~10 brand controls (real homepage = control group proving pages index) + ~90 winnable (homepage empty or social-only, weak competition we can rank #1 for). Pure logic in `src/stationPages/` (slug/select/stationIndex/render/sitemap), unit-tested mirroring `ogMeta.ts`. Committed `functions/station-index.json` (slim uuid→name/slug/country/genre/bitrate/favicon/related). **Why 100 not 14k:** low-authority domain has small crawl budget; a controlled subset gives a real index-rate signal + interpretable result (brand control isolates technical vs ranking failure) without false-negative "Discovered – not indexed" noise. **Decision gate ~3–4 weeks out:** indexed count / non-branded impressions / clicks → go/no-go on **batch 2** (full-catalog long-tail emphasis + dedicated country/genre hub pages + share-button migration to `/radio/...`). No change to `catalog.json`; existing `?station=` deep links unchanged. Spec: `docs/superpowers/specs/2026-06-27-programmatic-seo-station-pages-design.md`; plan: `docs/superpowers/plans/2026-06-27-programmatic-seo-station-pages.md` |
| Programmatic SEO — data quality + non-music withdrawal | ✅ Done — **response to the batch-1 GSC result: ~113 impressions, 0 clicks.** All 103 sitemap URLs indexed (100%), so indexing was never the problem; the queries were. Four language-independent fixes shipped. (1) **`isMusicStation` matched tags by SUBSTRING** — any station whose tag string merely contained "classical" was admitted; now exact-match with an explicit non-music exclusion list. (2) **Five Quran-recitation / religious-talk stations withdrawn** — they are not music and do not belong in a music discovery product; `/radio/[slug]` returns **410 Gone** (not 404) so Google drops the indexed URLs deliberately rather than re-crawling them as soft errors; index 100→95, sitemap 103→98, related links to them stripped so no page links to a 410. (3) **Genre corrections** via `genreOverrides.ts` (uuid→genre, applied at render time so re-running `buildStationPages.ts` cannot churn indexed URLs): Mosaique FM is Tunisia's generalist news/talk not classical, Jazz Radio Classic Jazz is jazz, Радио Шансон is chanson not pop, plus junk labels (`Amg`, `Aac`, `Best music for you`, bare `Music`). (4) **Station-name sanitization** via `stationName.ts`: `Люкс FМ` carried a Cyrillic М (U+041C) unmatchable by any Latin "FM" search; titles carried `- 128kb/s`, `[aac]`, `(320k MP3)`, callsign debris; stray leading punctuation. Deliberately conservative — `.977 Smooth Jazz` is branded with that dot and is preserved. Slugs come from the stored field and are never recomputed, so no indexed URL moves. **Verified:** 62/95 pages byte-identical, the 33 changed are exactly those with a corrected genre or cleaned name; 612 tests pass. |
| Programmatic SEO — localization (batch 1) | ⏸️ Parked, not abandoned — **the work exists in branch `feat/station-page-localization` (PR #38 history); read this before restarting.** ~90% of batch-1 impressions are Turkish and Russian *navigational* queries (`virgin radio dinle`, `радио маяк слушать`) served an English `<title>` on a `lang="en"` page, so 40 stations were localized into tr/ru/uk/fr/ar with the other 60 held as an English control. **Five independent per-locale reviews all returned "do not ship"**, finding: a French gender error (`radio Alternatif` — `radio` is feminine) on the Radio Nova page, the highest-volume French query; ungrammatical Turkish (`pop radyo` needs the possessive compound `pop radyosu`); `Türkü` (a specifically *Turkish* folk song) rendering on a Romanian station via a cross-border related row; `Російська естрада` on Ukrainian pages; and music-visualizer copy (`مؤثرات بصرية تتفاعل مع الصوت`) attached to Quran recitation in the footer of every Arabic page. **Four faults were structural, and the fixes are worth keeping:** (a) a genre label needs TWO forms per locale — a standalone `chip` for facts lines and a grammatically-shaped lowercase `inline` for prose (`radio d'information`, not `radio Infos`); (b) genre labels must not assert a nationality; (c) devotional content needs its own copy path (resolved instead by withdrawing those stations); (d) station names need sanitizing before interpolation. **Why parked:** fixing the findings surfaced that the copy was a *translation of English marketing prose* and read as translationese in every language regardless of engine — a Google Translate comparison came out worse (more literal, and it mistranslated "adult contemporary radio station" into adjectives agreeing with the station). Decisive point is allocation: the only verifiable locale (pt-BR — one station, zero observed impressions) and the valuable ones (tr/ru, ~90% of impressions) are disjoint. **If resumed:** transcreate from a brief rather than translate, cut the prose surface to title + description + facts (the only text a searcher sees), and get native review — ~30 short strings per locale. **But first settle whether the page type earns clicks at all**, since localization addresses only one of the three reasons for zero clicks; intent mismatch (`virgin radio frekans adana` wants an FM frequency) and authority against station-owned domains are untouched by language. |
| Programmatic SEO — batch 2, discovery-intent hub pages | 📋 Designed, not implemented — spec: `docs/superpowers/specs/2026-07-31-batch-2-discovery-hub-pages-design.md`. **Corrected selection criterion** after batch 1: demand alone or low-competition alone is not enough; a page needs **real search demand AND Spectrale being a legitimately good answer**. Batch 1 selected on low competition, which turned out to be the same thing as zero demand. **11 pages:** 3 *wedge* (`/internet-radio-with-visualizer`, `/music-visualizer-online`, `/ambient-radio-with-visuals`) where the audio-reactive WebGL makes us plausibly the best answer; 6 *mood* hubs (ambient, chillout, lounge, focus, sleep, relaxing) over curated tag groups; 2 *genre* controls (jazz, classical) that are **not expected to rank** but make a flat result interpretable — if the controls also flat-line, the constraint is domain authority and the answer is backlinks, not pages. **Excluded deliberately:** lofi (21 stations, query owned by YouTube/Spotify) and downtempo (52% overlap with chillout). **Hard constraint:** wedge pages list `corsFriendly` stations only — the visualizer does not run without CORS, and a page selling the visualizer must not send visitors where it silently degrades (8,935/14,119 qualify). **Anti-cannibalization:** the six mood pools share one ~544-station calm pool with 25–55% pairwise tag overlap; rule is no two hub pages share >40% of *listed* stations, achieved by a used-penalty in the ranking — verified at **0% overlap** on the real catalogue at 24 stations each, enforced by test. **Tags are whitelisted, never pattern-matched** (batch 1's root defect was a substring match, and the tag data is filthy — `moi merino` is the 11th most common tag). **New app capability:** `/?mood=<slug>` seeds the existing `activeFilter` before `loadInitial`, so a hub page's CTA actually starts a matching station; `?station=` keeps precedence. `MOODS` map in `src/hubPages/moods.ts` is the single source of truth shared by generator and app. Lists generated at build time into a committed `functions/hub-index.json` (`npm run build:hub-pages`, separate from `build:station-pages` so hubs cannot churn the 95 indexed station URLs); HTML rendered per request by a Pages Function. Sitemap 98 → 109. **Gate ~2026-09-11, read by page type — impressions do not count, clicks do.** |
