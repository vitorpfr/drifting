...
| Programmatic SEO — station-page localization (batch 1) | ✅ Done — **response to the batch-1 GSC result: ~113 impressions, 0 clicks.** All 103 sitemap URLs indexed (100%), so indexing was never the problem; the queries were. ~90% of impressions are Turkish and Russian *navigational* station queries (`virgin radio dinle`, `радио маяк слушать`, `baba radyo dinle`, `шансон`) being served an English `<title>` on a `lang="en"` page. **Scope:** full-page localization (head + all visible chrome), not head-only — a localized title leading to an English body trades a bounce for the click, and Google re-derives `lang` from rendered text anyway. **Cohorts:** 40 stations localized into the 5 languages we have observed impressions in (tr 4, ru 8, uk 9, fr 10, ar 9); the other 60 stay English as a **control** for measuring CTR lift. `src/stationPages/locale.ts` maps country→language with per-station `LOCALE_OVERRIDES` (Maghreb is too coarse by country: `Hits 1 Algérie` is French-language, `Mosaique FM` draws Latin-script queries). `src/stationPages/i18n/` holds one typed `Strings` table per locale — a missing key is a compile error, not a silent English leak. **Grammar constraints (documented in each locale file):** ru/uk place genre and country in appositive slots (`Жанр: X.`) because inline modifiers need adjective agreement + genitive case a template can't derive; tr avoids case suffixes on interpolated proper nouns (vowel harmony makes `Türkiye'dan` wrong); fr keeps genre parenthetical and country out of prepositional position (article selection); ar renders `dir="rtl"`, with `.dot`/`.hq` switched to logical margins so the layout mirrors. **Country names generated, not translated** — `scripts/buildCountryNames.ts` emits `src/stationPages/countryNamesI18n.json` from ICU via `Intl.DisplayNames` (26 countries × 6 locales), so the Worker needs no runtime ICU and related rows render in the *viewing* page's language. **`genreOverrides.ts`:** radio-browser's substring `isMusicStation` check admitted five Quran-recitation / religious-talk stations tagged `classical`; harmless while unread in English, but translated into fluent Arabic it becomes a confident falsehood aimed at the exact cohort being targeted. Also corrects `Радио Шансон` (chanson is its own genre, not pop), `Mosaique FM` (generalist, not classical), `Tes3enat FM` / `قديم FM` (classic hits), and junk labels (`Amg`, `Aac`, `Best music for you`, bare `Music`). Applied at render time, not index-build time — re-running `buildStationPages.ts` re-selects from the live API and would churn the 100 already-indexed URLs. **Verified:** all 60 English control pages byte-identical across title/description/h1/facts/lead/CTA/footer (only related-row genre labels shift, where they point at a corrected station); 40/40 localized pages changed; 667 tests pass. **Open risk:** no native-speaker review of the tr/ru/uk/fr/ar tables — strings are deliberately short and formulaic, but a spot-check on Turkish and Russian (the two cohorts carrying actual impressions) would de-risk before the decision gate. **Measurement:** compare localized vs English-control CTR over 4–6 weeks; feeds the same batch-2 decision gate. |
