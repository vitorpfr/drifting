...
| Feature | Status |
|---|---|
| Launch experience | ✅ Done — wordmark centered; tagline "Internet radio, alive with color" (always shown, including deep links); CTA "click/tap anywhere to start listening" (bottom 22%, bold uppercase) replaced by "click/tap to play [Station Name]" on deep links; ambient visualization starts at higher speed (0.8) and contrast/drama (2.0) on landing, settles smoothly to normal (speed 0.30, drama 1.0) over 3.5s on first interaction; on tap, wordmark disappears and skeleton shimmer + real controls appear immediately |
...
| Gesture model | ✅ Done — all four directions + keyboard (←→↑↓ Space S F , . ? [ ]) + on-screen chevrons + touch swipe recognition (`useSwipeGesture` hook: 50px H / 80px V thresholds, left-edge dead zone, scroll prevention, pointer capture, flash feedback); `[` / `]` adjusts volume ±10% (clamped, desktop only); hold effects deferred |
...
| Ripple rings | ✅ Done — `RippleEffect`: 4-slot rings; CORS stations beat-driven (EMA onset detector, threshold 0.04, cooldown 250ms); non-CORS 5s autonomous pulse; CORS stations stop the autonomous pulse so rings are beat-only (no doubling); Ambient ring duration 1.5s (`RING_SPEED_AMBIENT = 1/1.5`), Drift 3s |
...
| Analytics instrumentation | ✅ Done — `analytics.ts` wrapper (`trackEvent`); Umami Cloud (EU region, cookie-free, no consent popup required); script loaded via `index.html`; 5 events: `playback_started` (once per session on first play), `playback_duration` (seconds on every navigation away), `station_skipped` (forward navigations), `station_saved`, `genre_seed_completed` (with genre count); console.log in dev, `window.umami.track()` in prod; replaced broken `__cfBeacon.report()` approach (CF Web Analytics has no custom events API) |
